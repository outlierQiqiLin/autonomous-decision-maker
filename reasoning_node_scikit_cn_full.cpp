...
public:
  ...
  : nh_(nh), pl_("/rosprolog", true)
  {
    ...
    pl_ = rosprolog::PrologClient(prolog_ns_, true);
    pl_.waitForServer();
...
private:
                        ...
                        world_percept_assig4::QueryKnowledge::Response& res)
  {
    ...
    if (any_nonzero) {
      ...
      res.candidate_places.clear();
      for (const auto& kv : scored) res.candidate_places.push_back(kv.first);

      ROS_INFO_STREAM("[reasoning] return candidates sorted by learned prob for target=" << target);
    } else {
      // 若 KB 里还没写概率，就按原顺序返回
      res.candidate_places = candidates;
